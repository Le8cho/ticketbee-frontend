...
describe('SearchProvider and CommandMenu', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    // El CommandMenu arma su listado de comandos a partir del sidebar del
    // rol autenticado — sin sesión no hay nada que buscar.
    useAuthStore.getState().auth.setSession('token-test', {
      id: 'u1',
      nombre: 'Ana Técnico',
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/tecnico/clientes' })
    await expect
      .element(screen.getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })
